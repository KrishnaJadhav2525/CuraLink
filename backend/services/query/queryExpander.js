...
export async function expandQuery(disease, userQuery) {
  try {
    ...
    logger.warn(`[QueryExpander] Ollama expansion failed: ${err.message}`);
  }

  if (GROQ_API_KEY) {
    try {
      const groqResult = await expandWithGroq(disease, userQuery);
      if (groqResult) return groqResult;
    } catch (err) {
      logger.warn(`[QueryExpander] Groq expansion failed: ${err.message}`);
    }
  }
...
async function expandWithGroq(disease, userQuery) {
  const prompt = buildExpansionPrompt(disease, userQuery);
  ...
  logger.info(`[QueryExpander] LLM produced ${valid.length} variants via Groq (${GROQ_MODEL})`);
  return valid;
}
