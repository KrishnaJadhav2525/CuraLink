...
export async function handleChat(req, res, next) {
  const { disease, query, patientName, location, conditions, sessionId: existingSessionId } = req.body;
  ...
  try {
    ...
    if (!userProfile) {
      ...
      const extraConditions = conditions.split(',').map(c => c.trim()).filter(Boolean);
      userProfile.conditions = [...new Set([...(userProfile.conditions || []), ...extraConditions])];
    }
