...
export default function Navbar() {
  ...
  return (
    <motion.nav
      initial={{ y: -100 }}
      animate={{ y: 0 }}
      className="fixed top-0 left-0 right-0 z-50 px-5 py-4 pointer-events-none"
    >
      <div className="max-w-6xl mx-auto flex items-center justify-between px-6 py-3 premium-card !rounded-2xl !bg-white/60 shadow-lg border-white/60 backdrop-blur-xl pointer-events-auto">
        ...
        <Link to="/" className="flex items-center gap-2 group">
          <div className="w-8 h-8 rounded-xl flex items-center justify-center bg-gradient-to-br from-teal-500 to-cyan-600 shadow-lg shadow-teal-500/30 group-hover:scale-110 transition-transform">
        ...
        {!isChat && (
          <div className="hidden md:flex items-center gap-8">
            {['Pipeline', 'Clinical Trials', 'Research Sources'].map(item => (
