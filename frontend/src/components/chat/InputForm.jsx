...
import { User, MapPin, Stethoscope, ArrowUp, Loader2, Search, Sparkles, Brain, Activity } from 'lucide-react';
...
export default function InputForm() {
  ...
  const [form, setForm] = useState({ patientName: '', disease: '', query: '', location: '', conditions: '' });
  ...
  const handleSubmit = async (e) => {
    ...
    setPatientContext({ patientName: form.patientName, disease: form.disease, location: form.location, conditions: form.conditions });
    await sendMessage({ query: form.query, disease: form.disease, patientName: form.patientName, location: form.location, conditions: form.conditions });
  };
      ...
      <div className="max-w-3xl mx-auto premium-card !p-8 !rounded-[2.5rem] border-white/60 relative">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
          {[
            ...
            { icon: Activity, field: 'conditions', placeholder: 'Other conditions...' },
        ...
        <div className="relative rounded-2xl transition-all duration-300 group"
          style={{
            ...
            border: `1px solid ${focused ? '#0d9488' : 'rgba(229, 231, 235, 0.5)'}`,
            boxShadow: focused ? '0 10px 25px -10px rgba(13, 148, 136, 0.15), 0 0 0 4px rgba(13, 148, 136, 0.03)' : 'none',
          }}>
          <textarea
            ref={textareaRef}
            rows={1}
          ...
          <div className="absolute bottom-4 left-5 right-4 flex items-center justify-between pointer-events-none">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11px] font-bold" style={{ color: form.disease ? '#0d9488' : '#4b5563' }}>
                {form.disease
